...
def pano_decode_32_reg(heat_center, tooth_wh=None, K=1):
    heat_center = _nms(heat_center)
    batch_size, cat, h, w = heat_center.size()
    detections = None

    for i in range(cat):
        scores_center, inds_center, _, ys_center, xs_center = _topk(heat_center[:, i:i+1, :, :], K=K)
        ...
        if tooth_wh is None:
            det = torch.cat([xs_center, ys_center, scores_center], dim=2)
        else:
            this_wh = _transpose_and_gather_feat(tooth_wh, inds_center)
            this_wh = this_wh.view(batch_size, K, 2)
            det = torch.cat([xs_center, ys_center, scores_center, this_wh], dim=2)

        if detections is None:
            detections = det
        else:
            detections = torch.cat([detections, det], dim=1)

    return detections

def pano_decode_1_class(heat_center, c, tooth_wh=None, K=100):
    batch_size, _, h, w = heat_center.size()
    heat_det = heat_center[:, c:c+1, :, :]
    heat_det = _nms(heat_det)

    scores_center, inds_center, _, ys_center, xs_center = _topk(heat_det, K=K)
    ...
    if tooth_wh is None:
        detections = torch.cat([xs_center, ys_center, scores_center], dim=2)
    else:
        this_wh = _transpose_and_gather_feat(tooth_wh, inds_center)
        this_wh = this_wh.view(batch_size, K, 2)
        detections = torch.cat([xs_center, ys_center, scores_center, this_wh], dim=2)
